...
def get_dataset(dataset=DATASET_DEFAULT, batch=5000, index=None, val=False):
    ...
    '''
    df = get_data_csv(dataset, val=val)
    ...
    return get_dataset_images(df, dataset, val=val)


def get_data_csv(dataset, val=False):
    '''
...
def get_dataset_images(df, dataset=DATASET_DEFAULT, val=False):
    '''
